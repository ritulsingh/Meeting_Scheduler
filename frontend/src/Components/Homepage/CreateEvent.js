...
const CreateEvent = () => {
    ...
    const submit = () => {
        const { email, title, agenda, time, guest, date } = user
        if (title && agenda && time && guest && date) {
            axios.post("http://localhost:9002/createevent", user)
                .then(res => {
            ...
            <aside className="sidenav navbar navbar-vertical navbar-expand-xs border-0 border-radius-xl my-2 fixed-left ms-4" id="sidenav-main">
                ...
                <ul className="navbar-nav">
                    <li className="nav-item mb-2">
                        <a className="nav-link" href="#" onClick={() => history("/dashboard")}>
                    ...
                    <li className="nav-item mb-2">
                        <a className="nav-link active" href="#" onClick={() => history("/createevent")}>
                            <div className="icon icon-shape icon-xs shadow border-radius-sm bg-gradient-light text-center me-2 d-flex align-items-center justify-content-center">
                                <svg width="12px" height="12px" viewBox="0 0 42 42" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
                                    <title>office</title>
                                    <g id="Basic-Elements" stroke="none" strokeWidth={1} fill="none" fillRule="evenodd">
            ...
            <main className="main-content border-radius-lg" >
                    ...
                    <div className="container-fluid px-3">
                        <nav aria-label="breadcrumb">
                            <ol className="breadcrumb bg-transparent mb-0 pb-1 px-0 me-sm-6 me-12">
                                <li className="breadcrumb-item"><a className="fs-2 text-dark" href="javascript:;">Create Your Event</a></li>
                            </ol>
                        </nav>
                    </div>
                </nav>
                ...
                <div className="mt-2 ">
                    <div className="container">
                        <p className="row" style={{ alignItems: 'center', justifyContent: 'center', color: 'red' }}>
                        </p>
                        <p className="row" style={{ alignItems: 'center', justifyContent: 'center', color: 'green' }}>
                        </p>
                    </div>
                    <div className="container">
                        <div className="row w-80">
                            <div className="col-sm">
                                <p className="font-weight-bold my-1 py-1">Title</p>
                                <input name="title" value={user.title} className="form-control" type="text" onChange={handleChange} />
                            </div>
                            <div className="col-sm">
                                <p className="font-weight-bold my-1 py-1">Agenda</p>
                                <input name="agenda" value={user.agenda} className="form-control" type="text" onChange={handleChange} />
                            </div>
                        </div>
                    </div>
                    <div className="container mt-3">
                        <div className="row w-80">
                            <div className="col-sm">
                                <p className="font-weight-bold my-1 py-1">Time</p>
                                <input name="time" value={user.time} className="form-control" type="time" onChange={handleChange} />
                            </div>
                            <div className="col-sm">
                                <p className="font-weight-bold my-1 py-1">Guest</p>
                                <input name="guest" value={user.guest} className="form-control" type="text" onChange={handleChange} />
                            </div>
                        </div>
                    </div>
                    <div className="container mt-3">
                        <div className="row w-80">
                            <div className="col-sm">
                                <p className="font-weight-bold my-1 py-1">Date</p>
                                <input name="date" value={user.date} className="form-control" type="date" onChange={handleChange} />
                            </div>
                        </div>
                    </div>
                    <div className="container mt-4">
                        <div className="row w-80">
                            <div className="col-sm align-self-end pb-1">
                                <button onClick={submit} className="btn btn-primary btn-sm mb-0">Submit</button>
                            </div>
                        </div>
                    </div>
                </div>
